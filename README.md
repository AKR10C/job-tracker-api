...
### 1. **Explain the data flow end-to-end**:
- User registers → Password hashed → Stored in DB
- User logs in → Password verified → JWT token created
- User adds job → Token verified → Job associated with user
- User retrieves jobs → Token verified → Return filtered jobs

### 2. **Be ready to discuss trade-offs**:
- SQLAlchemy: Powerful but slower than raw SQL
- JWT: Stateless but harder to revoke
- PBKDF2: Secure but slower than modern algorithms

### 3. **Common follow-up questions**:
- "How would you handle refresh tokens?" (Separate endpoint returning new JWT)
- "How would you rate limit?" (Middleware or FastAPI middleware)
- "How would you handle multiple users?" (Already built-in with user_id filtering)
- "How would you scale this?" (Stateless design allows horizontal scaling)

### 4. **Code quality improvements to mention**:
...
| **SQLAlchemy** | ORM | Tortoise ORM, Peewee |
| **Pydantic** | Data validation | Marshmallow, Cerberus |
| **passlib** | Password hashing | bcrypt alone |
| **python-jose** | JWT handling | PyJWT |
| **PyMySQL** | MySQL driver | mysqlclient |
| **uvicorn** | ASGI server | Hypercorn, Daphne |
...
1. **REST API Design**: HTTP methods, status codes, endpoints
2. **Authentication**: Hashing, salting, tokens, JWT
3. **Databases**: SQL basics, ORM patterns, relationships
4. **Python**: Type hints, decorators, context managers
5. **Web Security**: CORS, SQL injection, password best practices

### Interview code patterns to know:
- Dependency injection
- Context managers (try/finally)
- Generators (yield)
- Decorators (@app.post, @app.get)
- Type hints (-> return type, param: type)
